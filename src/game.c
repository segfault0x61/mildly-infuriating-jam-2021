...
void game_update(double delta) {
    ...
    if (keys[SDL_SCANCODE_W] || keys[SDL_SCANCODE_UP]) {
        if (player.is_bat) {
            player.y_vel =
                MAX(player.y_vel - (accel * 1.2f), -(max_imp * accel * 1.2f));
            moving_on_up = true;
        }
    } else if (keys[SDL_SCANCODE_S] || keys[SDL_SCANCODE_DOWN]) {
        if (player.is_bat) {
            player.y_vel = MIN(player.y_vel + accel, max_imp * accel);
        }
        ...
        if (fabs(player.y_vel - BAT_FLOATING_VEL) < FLT_EPSILON) {
            ...
            player.y_vel -=
                MIN(fabs(player.y_vel - BAT_FLOATING_VEL), accel / 2.0f) * sign;
        }
    }

    if (keys[SDL_SCANCODE_A] || keys[SDL_SCANCODE_LEFT]) {
        player.x_vel = MAX(player.x_vel - accel, -(max_imp * accel));
        player_s->flip_mode = SDL_FLIP_HORIZONTAL;
    } else if (keys[SDL_SCANCODE_D] || keys[SDL_SCANCODE_RIGHT]) {
        player.x_vel = MIN(player.x_vel + accel, max_imp * accel);
    ...
    if (player.is_bat || fabs(player.x_vel) > FLT_EPSILON) {
        ...
        if (player.anim_timer > 150) {
            player_s->cur_frame =
                (player_s->cur_frame + 1) % player_s->num_frames;
            player.anim_timer = 0;
        }
    } else {
