...
void sprite_set_tex(Sprite* s, const char* name, int frames) {
    ...
    if (!tex) {
        ...
        TexCache tc = {.name = strdup(name), .tex = tex};
...
Sprite* sprite_push_tex_frames(int x, int y, int w, int h, const char* name,
                               int frames) {
    if (num_sprites >= array_count(sprites)) {
        return NULL;
    }
...
SDL_Point sprite_get_center(Sprite* s) {
    SDL_Point p = {s->x + (s->w / 2), s->y + (s->h / 2)};
...
void sprite_draw(Sprite* s) {
    ...
    if (s->tex) {
        ...
        SDL_RenderCopyEx(renderer, s->tex, src_rect_ptr, &s->rect, s->rotation,
                         NULL, s->flip_mode);
    } else {
